...
const PaymentPage = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'cardNumber') {
      ...
      setFormData(prev => ({ ...prev, expiryDate: clean.length > 2 ? clean.slice(0,2) + '/' + clean.slice(2) : clean }));
    } else if (name === 'cvv') {
  ...
  const validateUPI = () => {
    if (!/^[a-zA-Z0-9._-]+@[a-zA-Z]{3,}$/.test(formData.upiId.trim())) {
      setError('Please enter a valid UPI ID (e.g., username@bankname)'); return false;
    }
    return true;
  };
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      if (confirmRes.data?.message) {
        ...
        // Update localStorage so home page can show "Cancel Ticket"
        localStorage.setItem('currentBooking', JSON.stringify({ ...booking, paymentStatus: 'Completed' }));
        ...
        // Redirect to HOME — user will see Cancel Ticket button there
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 py-8 sm:py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <button onClick={() => navigate('/booking/' + booking.flight?._id)}
        ...
        {success && (
          <div className="mb-6 bg-green-50 border-l-4 border-green-500 rounded-lg p-4 flex gap-3">
            ...
            <div>
              ...
              <p className="text-green-700 text-xs mt-1">A confirmation email has been sent. Redirecting to home…</p>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 sm:p-8">
            ...
            <div className="grid grid-cols-3 gap-2 mb-6">
              {[
                ...
                <button key={key} onClick={() => { setPaymentMethod(key); setError(''); }}
            ...
            {paymentMethod === 'card' && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input type={showCVV ? 'text' : 'password'} name="cvv" value={formData.cvv} onChange={handleInputChange}
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <p className="text-xs text-blue-800 font-semibold">💳 Test Card: 4242 4242 4242 4242 | Any future date | Any CVV</p>
                </div>
              </div>
            )}
            ...
            {paymentMethod === 'upi' && (
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <p className="text-xs text-blue-800 font-semibold">📱 Supported: Google Pay, PhonePe, Paytm, WhatsApp Pay, BHIM</p>
                </div>
              </div>
            )}
            ...
            {paymentMethod === 'bank' && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" name="accountHolderName" value={formData.accountHolderName} onChange={handleInputChange}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl shadow-lg border-2 border-green-200 p-6 sm:p-8">
              ...
              <div className="space-y-3 mb-6">
                {['Secure SSL encrypted connection','PCI DSS compliant payment processing','Money-back guarantee if booking cancels','24/7 customer support available'].map(t => (
              ...
              <button onClick={processPayment} disabled={processing || !!success}
                ...
                {processing ? (<><Loader2 className="animate-spin" size={20} /><span>Processing Payment…</span></>)
                 : success   ? (<><CheckCircle size={20} /><span>Payment Successful!</span></>)
